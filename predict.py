...
# Base checkpoint: AnIllustrious v5 (Civitai model 1121885, version 1652528, by
# Jedas), mirrored in diffusers format on the Hub by John6666. Civitai's own
# /api/download route no longer accepts API-key auth from plain HTTP clients
# (redirects to /login regardless of token, even for non-gated files), so we
# pull the public HF mirror instead of the original SDXL tar fofr's cog used.
# License: faipl-1.0-sd (https://freedevproject.org/faipl-1.0-sd/). See README.
ILLUSTRIOUS_REPO_ID = "John6666/anillustrious-v5vae-sdxl"
REFINER_URL = (
...
class Predictor(BasePredictor):
    ...
    def setup(self, weights: Optional[Path] = None):
        ...
        WeightsDownloader.download_hf_snapshot_if_not_exists(
            ILLUSTRIOUS_REPO_ID, SDXL_MODEL_CACHE
        )

        print("Loading illustrious txt2img pipeline...")
